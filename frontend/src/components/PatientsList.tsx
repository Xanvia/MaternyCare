...
const PatientsList: React.FC<PatientsListProps> = ({ mothers }) => {
  ...
  const getLatestAppointment = (appointments: Mother["appointments"]) => {
    const withFixedDate = appointments
      .filter((appointment) => appointment.fixedDate)
      .sort((b, a) => new Date(b.fixedDate).getTime() - new Date(a.fixedDate).getTime());

    if (withFixedDate.length > 0) return withFixedDate[0]; // Return latest based on fixedDate

    const withStartDate = appointments
      .sort((b, a) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());

    return withStartDate[0] || null; // Return latest based on startDate or null
  };
                        ...
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                          {latestAppointment ? (
                            <div>
                              ...
                              {latestAppointment.fixedDate ||
                                  latestAppointment.startDate}
                              </p>
                            </div>
                          ) : (
                            <p className="text-gray-500 whitespace-no-wrap">
